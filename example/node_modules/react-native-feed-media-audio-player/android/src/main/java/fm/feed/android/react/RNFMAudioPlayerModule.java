...
public class RNFMAudioPlayerModule extends ReactContextBaseJavaModule implements FeedAudioPlayer.StateListener,
  ...
  @ReactMethod
  public void setActiveStation(Double activeStation) {

    String station = activeStation.toString();
    boolean flag = false;
    for (Station st: mFeedAudioPlayer.getStationList()) {

      if(st.getId().toString().equals(station))
      {
    ...
    if(!flag)
    {
      Log.e("Feed", "Cannot set active station to "+activeStation+" because no station found with that id");
    }
  }
